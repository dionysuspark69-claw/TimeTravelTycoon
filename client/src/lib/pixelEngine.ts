...
// ── Game era aliases ───────────────────────────────────────────────────────
// The base engine ships 23 strata. The game has many more era IDs; map each
// extra ID to an existing era's palette / terrain / recipe / landmark, and
// insert it into STRATA at a chronologically reasonable position so the
// cross-section viewport and depth map work for every destination.
interface AliasSpec {
  alias: string;          // source era to copy palette/terrain/recipe from
  label: string;          // display label
  depth: string;          // depth marker shown in stratum tab
  landmark: LandmarkKind; // landmark silhouette
  after: string;          // STRATA era key to insert after (top→bottom = future→past)
}
const ERA_ALIASES: Record<string, AliasSpec> = {
  farfuture:    { alias: "future",      label: "Far Future",       depth: "+5000",  landmark: "spire",      after: "future" },
  postscarcity: { alias: "paradise",    label: "Post-Scarcity",    depth: "+4500",  landmark: "palms",      after: "paradise" },
  singularity:  { alias: "aiutopia",    label: "Singularity",      depth: "+3500",  landmark: "spire",      after: "aiutopia" },
  galactic:     { alias: "future",      label: "Galactic",         depth: "+2800",  landmark: "monorail",   after: "future" },
  interstellar: { alias: "spaceage",    label: "Interstellar",     depth: "+2500",  landmark: "rocket",     after: "mars" },
  jupiter:      { alias: "spaceage",    label: "Jupiter Orbit",    depth: "+2300",  landmark: "moondome",   after: "mars" },
  asteroid:     { alias: "mooncolony",  label: "Asteroid Belt",    depth: "+2200",  landmark: "moondome",   after: "mooncolony" },
  postapoc:     { alias: "wildwest",    label: "Post-Apocalypse",  depth: "+2150",  landmark: "smokestack", after: "cyberpunk" },
  digital:      { alias: "cyberpunk",   label: "Digital Age",      depth: "+2000",  landmark: "tower",      after: "spaceage" },
  moonlanding:  { alias: "spaceage",    label: "Moon Landing",     depth: "+1969",  landmark: "rocket",     after: "spaceage" },
  ww2:          { alias: "industrial",  label: "World War II",     depth: "+1945",  landmark: "smokestack", after: "roaring20s" },
  dieselpunk:   { alias: "industrial",  label: "Dieselpunk",       depth: "+1940",  landmark: "smokestack", after: "roaring20s" },
  ww1:          { alias: "industrial",  label: "World War I",      depth: "+1918",  landmark: "smokestack", after: "roaring20s" },
  steampunk:    { alias: "industrial",  label: "Steampunk",        depth: "+1880",  landmark: "smokestack", after: "wildwest" },
  victorian:    { alias: "industrial",  label: "Victorian",        depth: "+1870",  landmark: "castle",     after: "wildwest" },
  americanrev:  { alias: "renaissance", label: "American Rev.",    depth: "+1776",  landmark: "castle",     after: "industrial" },
  frenchrev:    { alias: "renaissance", label: "French Rev.",      depth: "+1789",  landmark: "castle",     after: "industrial" },
  discovery:    { alias: "renaissance", label: "Age of Discovery", depth: "+1500",  landmark: "longboat",   after: "renaissance" },
  aztec:        { alias: "egypt",       label: "Aztec Empire",     depth: "+1400",  landmark: "pyramid",    after: "renaissance" },
  inca:         { alias: "atlantis",    label: "Inca Empire",      depth: "+1400",  landmark: "pyramid",    after: "renaissance" },
  mongol:       { alias: "viking",      label: "Mongol Empire",    depth: "+1250",  landmark: "longboat",   after: "medieval" },
  feudaljapan:  { alias: "medieval",    label: "Feudal Japan",     depth: "+1200",  landmark: "castle",     after: "medieval" },
  mayan:        { alias: "atlantis",    label: "Mayan",            depth: "+0900",  landmark: "pyramid",    after: "viking" },
  camelot:      { alias: "medieval",    label: "Camelot",          depth: "+0500",  landmark: "castle",     after: "viking" },
  china:        { alias: "egypt",       label: "Ancient China",    depth: "+0100",  landmark: "castle",     after: "rome" },
  greece:       { alias: "rome",        label: "Ancient Greece",   depth: "−0400",  landmark: "columns",    after: "rome" },
  olympus:      { alias: "rome",        label: "Mount Olympus",    depth: "−0600",  landmark: "columns",    after: "rome" },
  mesopotamia:  { alias: "egypt",       label: "Mesopotamia",      depth: "−3000",  landmark: "pyramid",    after: "egypt" },
  bronzeage:    { alias: "egypt",       label: "Bronze Age",       depth: "−2000",  landmark: "pyramid",    after: "egypt" },
  stoneage:     { alias: "prehistoric", label: "Stone Age",        depth: "−8000",  landmark: "cave",       after: "prehistoric" },
  iceage:       { alias: "viking",      label: "Ice Age",          depth: "−20K",   landmark: "cave",       after: "prehistoric" },
  triassic:     { alias: "dinosaur",    label: "Triassic",         depth: "−230M",  landmark: "trex",       after: "dinosaur" },
  cambrian:     { alias: "atlantis",    label: "Cambrian",         depth: "−540M",  landmark: "atlantis",   after: "dinosaur" },
};

for (const [id, spec] of Object.entries(ERA_ALIASES)) {
  if (!ERA_PALETTES[id])  ERA_PALETTES[id]  = ERA_PALETTES[spec.alias];
  if (!ERA_META[id])      ERA_META[id]      = { label: spec.label, terrain: ERA_META[spec.alias].terrain };
  if (!CHAR_RECIPES[id])  CHAR_RECIPES[id]  = CHAR_RECIPES[spec.alias];
  if (!STRATA.some(s => s.era === id)) {
    const anchorIdx = STRATA.findIndex(s => s.era === spec.after);
    const insertAt = anchorIdx < 0 ? STRATA.length : anchorIdx + 1;
    STRATA.splice(insertAt, 0, { era: id, depth: spec.depth, landmark: spec.landmark });
  }
}
